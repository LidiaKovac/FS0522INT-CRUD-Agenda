...
window.onload = async () => {
  try {
    if (id !== null) {
      ...
      if(res.ok) {
        let { name, description, price, time } = await res.json()
        ...
        document.querySelector("#eventTime").value = time.replace(".000Z", "")
  ...
  try {
    ...
    const options = {
      headers: new Headers({
        ...
        "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2MGFlNDE0YWNlYWY0ODAwMTVjOTE4NzAiLCJpYXQiOjE2NzQxMjgyMDUsImV4cCI6MTY3NTMzNzgwNX0.-tYdinRFC9eXeHov6wpQFoMFEL7uxwz2sSlordR-gjQ"
      }),
